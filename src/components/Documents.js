...
const Documents = () => {
  ...
  const fetchDocuments = async () => {
    try {
      const response = await axios.get(`${urll}show_documents/`);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = formData.document_id
      ? `${urll}insert_update_documents/?pk=${formData.document_id}`
      : `${urll}insert_update_documents/`;
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`${urll}delete_document/?document_id=${deleteId}/`);
      setMessages(response.data.message);
      fetchDocuments();
      closeDeleteModal();
    } catch (err) {
        ...
        <tbody>
          {documents.map((doc) => (
            <tr key={doc.document_id}>
              ...
              <td><Link to={`${urll}media/${doc.document_file}`}>Open</Link></td>
              <td>
                <button
