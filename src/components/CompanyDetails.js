...
const CompanyDetails = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.post(
        `${urll}insert_update_comapny_detail/`,
        formData,
        {
  ...
  <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-col">
        <label className="text-gray-700 font-medium">સરૂઆતી શીલક</label>
        {editMode ? (
          <input
            type="number"
            name="company_sharuaati_shilak"
            value={updatedData.company_sharuaati_shilak || ""}
            onChange={handleChange}
            className="border rounded-lg p-2 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        ) : (
          <p className="text-gray-600">{companyData.company_sharuaati_shilak}</p>
        )}
      </div>
