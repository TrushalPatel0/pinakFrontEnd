...
const CreateUser = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(`${urll}show_user/`);
  ...
  const editDetails = async (id) => {
    try {
      const response = await axios.get(
        `${urll}insert_update_user/?getdata=${id}`
      );
  ...
  const deleteUser = async () => {
    try {
      await axios.delete(`${urll}delete_user/?user_id=${delId}`);
